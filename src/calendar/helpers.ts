...
export function calculateMonthEventPositionsForGrid(multiDayEvents: IEvent[], singleDayEvents: IEvent[], gridStart: Date, gridEnd: Date) {
  const eventPositions: { [key: string]: number } = {};
  const occupiedPositions: { [key: string]: boolean[] } = {};

  // Initialize occupied positions for the entire calendar grid range
