  ...
  <div class="grid min-h-[768px] grid-cols-[1fr_280px] overflow-hidden">
    ...
    <div class="grid grid-rows-[auto_1fr] overflow-hidden">
      ...
      <div class="sticky top-0 z-20 bg-background">
            ...
            <div
              :class="
                cn(
                  ...
                  isToday(selectedDate) && 'mx-auto flex h-8 w-8 items-center justify-center rounded-full bg-primary text-primary-foreground'
                )
              "
            >
        ...
        <!-- All-day events row -->
        <div v-if="allDayEvents.length > 0" class="flex border-b border-border bg-background">
          ...
          <div class="flex-1 space-y-1 p-2">
            <div v-for="event in allDayEvents" :key="event.id" class="cursor-pointer" @click="handleEventClick(event)">
      ...
      <ScrollArea class="min-h-0">
        <div class="flex">
          ...
          <div class="w-18 border-r border-border">
            <div v-for="(hour, index) in hours" :key="hour" class="relative border-b border-border" style="height: 96px">
              <div v-if="index !== 0" class="absolute -top-3 right-2 flex h-6 items-center">
          ...
          <div class="relative flex-1">
            ...
            <div
              v-for="hour in hours"
              :key="hour"
              :class="cn('relative border-b border-border', !isWorkingHour(selectedDate, hour, workingHours) && 'bg-muted/30')"
              style="height: 96px"
            >
              <!-- Half hour line -->
              <div v-if="hour !== hours[hours.length - 1]" class="absolute inset-x-0 top-1/2 border-b border-dashed border-border/50"></div>
            ...
            <!-- Events -->
            <div v-for="(group, groupIndex) in eventGroups" :key="`group-${groupIndex}`">
              <div v-for="event in group" :key="event.id" class="absolute w-full p-1" :style="getEventStyle(event, groupIndex, eventGroups.length)">
                <DayEventBlock :event="event" @event-click="handleEventClick" />
              </div>
            </div>

            <!-- Current time indicator -->
            <CalendarTimeline v-if="isToday(selectedDate)" :first-visible-hour="earliestEventHour" :last-visible-hour="latestEventHour" />
          </div>
        </div>
      </ScrollArea>
    </div>
    ...
    <div class="border-l border-border bg-background p-4">
      <div class="mb-4">
        <Calendar class="rounded-md border border-border" mode="single" :model-value="selectedCalendarDate" @update:model-value="handleDateChange" />
      </div>
    </div>
  </div>
</template>
...
import { format, isToday, isSameDay, parseISO, areIntervalsOverlapping } from "date-fns";
...
const { selectedDate, workingHours, visibleHours, setSelectedDate } = useCalendar();
...
const handleDateChange = (calendarDate: CalendarDate | undefined) => {
  if (calendarDate) {
    const newDate = new Date(calendarDate.year, calendarDate.month - 1, calendarDate.day);
...
const dayEvents = computed(() =>
  props.singleDayEvents.filter(event => isSameDay(parseISO(event.startDate), selectedDate.value) || isSameDay(parseISO(event.endDate), selectedDate.value))
);
...
const allDayEvents = computed(() =>
  props.multiDayEvents.filter(
    event =>
      ...
      (parseISO(event.startDate) < selectedDate.value && parseISO(event.endDate) > selectedDate.value)
  )
);

// Time calculations
const visibleHoursData = computed(() => getVisibleHours(visibleHours.value, dayEvents.value));
...
const formatHour = (hour: number) => format(new Date().setHours(hour, 0, 0, 0), "h a");

const getEventStyle = (event: IEvent, groupIndex: number, groupSize: number) => {
  let style = getEventBlockStyle(event, selectedDate.value, groupIndex, groupSize, { from: earliestEventHour.value, to: latestEventHour.value });
