<template>
  <div role="button" tabindex="0" :class="eventBlockClasses" :style="{ height: `${heightInPixels}px` }" @click="handleClick" @keydown="handleKeyDown">
    ...
    <p v-if="durationInMinutes > 25" class="text-xs opacity-90">{{ formatTime(start) }} - {{ formatTime(end) }}</p>
  </div>
</template>
...
const dayEventBlockVariants = cva(
  "flex select-none flex-col gap-0.5 truncate whitespace-nowrap rounded-md border px-2 py-1.5 text-xs focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring cursor-pointer hover:shadow-md transition-all duration-200 hover:scale-[1.02]",
  {
    variants: {
      color: {
...
const heightInPixels = computed(() => Math.max((durationInMinutes.value / 60) * 96 - 8, 20));

const color = computed(
  () => (badgeVariant.value === "dot" ? `${props.event.color}-dot` : props.event.color) as VariantProps<typeof dayEventBlockVariants>["color"]
);

const eventBlockClasses = computed(() =>
  cn(dayEventBlockVariants({ color: color.value }), props.className, durationInMinutes.value < 35 && "py-1 justify-center")
);
